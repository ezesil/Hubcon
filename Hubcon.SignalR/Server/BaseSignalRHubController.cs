...
namespace Hubcon.SignalR.Server
{
    public abstract class BaseHubController : Hub, IHubconServerController
    {
        ...
        public async Task HandleStream(MethodInvokeRequest info) => await MethodHandler.HandleStream(info);
        public async Task ReceiveStream(string code, ChannelReader<object> reader) => await StreamHandler.NotifyStream(code, reader);
